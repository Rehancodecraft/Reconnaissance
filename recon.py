#!/usr/bin/env python3
"""
osint_recon.py - Automated OSINT Reconnaissance Script (PDF Edition)
=====================================================================

Objective:
    Perform comprehensive OSINT reconnaissance on a target domain using
    a chain of well-known open-source tools, auto-installing any that
    are missing, and produce a single structured PDF report with tables.

Tasks automated (per lab spec):
    0. Dependency Check & Auto-Install -> installs any missing tool automatically
    1. Subdomain Discovery             -> subfinder, amass, assetfinder, sublist3r, crt.sh
    2. Live Host Validation             -> httpx-toolkit
    3. CDN Detection                    -> cdncheck
    4. Load Balancer Discovery          -> lbd + custom DNS check
    5. Technology Fingerprinting        -> whatweb, nmap (wafw00f skipped - not installed)
    6. Report Generation                -> PDF report (target findings only, live subdomains only)

Usage:
    python3 osint_recon.py -d example.com
    python3 osint_recon.py -d example.com -o /path/to/output_dir
    python3 osint_recon.py -d example.com --timeout 120
    python3 osint_recon.py -d example.com --no-install     # skip auto-install, just skip missing tools
...
IMPORTANT / LEGAL:
    ...
    Auto-install runs real package-manager / go-install commands with
    sudo - review INSTALL_COMMANDS below before running on a machine
    you don't fully control.

Design notes (why it's written this way, for easy troubleshooting):
    - Every external tool call goes through one helper: run_cmd().
      If a tool isn't installed, or times out, or errors, that step
      is skipped gracefully and logged -- the whole script never
      crashes because one tool is missing.
    - A global COMMANDS_LOG list records every command actually
      executed (phase, tool, command, success). This is printed live
      to the console for troubleshooting, but is intentionally NOT
      included in the PDF report - the report is meant to describe
      the target's exposure, not the tooling used to find it.
    - Each phase is its own function that returns a plain Python
      data structure (list/dict), used both for live console output
      and for building the PDF tables - one source of truth.
    - All raw tool output is passed through strip_ansi() before it is
      stored/displayed, since tools like whatweb/httpx-toolkit print
      ANSI color codes that look fine in a terminal but show up as
      garbage characters (e.g. "[1m[34m") inside a PDF.
"""
